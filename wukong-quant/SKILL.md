...
### 外资投行前十大流通股东——Q1/Q2 报告期区分

| 报告期 | period 参数 |
|--------|------------|
| 2026Q1（一季报） | `20260331` |
| 2026Q2（中报/H1） | `20260630` |
| 2026Q3（三季报） | `20260930` |
| 2026 年报 | `20261231` |

用户说"2026Q2 外资退出" → `period=20260630`，自动对比 `20260331`。

**数据局限性**：报告期要等披露完成才有数据（Q2 通常 8 月后才完整）。数据未就绪时工具会返回空结果或缺该期，
此时退回已披露的最近一期并说明。注意区分：`not connected` 是 transport 层故障，与该期数据是否披露无关，
不要把连接错误当成「数据还没出」，按下方「MCP 断连自愈」处理。

### 响应格式规范——外资股东筛选类查询

当 `matched_count` 很大时：
- 先按 `by_holder` 总结各机构命中数量
- 结果表只展示前 30-50 条（涨幅从低到高排序）
- 直接列出数据表，不加"以下是分析结果"之类的废话
- `matched_count` 要告知用户，如"共 1875 条，显示前 50 条"

**错误示范**：大段解释、数据来源说明、过程叙述。
**正确示范**：标题 → 数据表 → 统计摘要。

### MCP 循环警告处理

当工具返回 `"This looks like a loop"` 警告时：
- **不要**用完全相同的参数重试同一工具
- 做法：换用其他 wukong 工具探连通性（如 `mcp_wukong_quant_read_ladder_daily`），或等 60s 再试原工具
- 相同参数第 2+ 次重试会触发 Hermes 内置的 loop 拦截

### MCP 断连自愈——重试策略

> 经验规则：等待 30-60s 可解决瞬时抖动；若超过 3 分钟仍失败，则不是普通断连，需检查服务端进程。

**已验证的失败模式**：

| 错误信息 | 含义 | 处理方式 |
|----------|------|----------|
| `"is not connected"` | MCP 进程存在但 transport 故障 | 等 30-60s 再试 |
| `"unreachable after N consecutive failures"` + `"Auto-retry available in ~Xs"` | 连续失败触发退避，Xs 后可重试 | 等待 Xs+5s 后重试 |
| 等了 120s+ 仍然 `not connected` | 服务端进程崩溃或端口未监听 | 不要继续重试，告知用户检查服务端 |

**等待上限**：连续等 3 轮（每轮 45-65s）仍未恢复，就不再是瞬时抖动，判定服务端已崩溃。此时停止轮询，直接告知用户需要检查或重启服务端。
